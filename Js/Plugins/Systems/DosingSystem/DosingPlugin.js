! function() {
    ...
    if (window.Plugs.DosingSystem) throw new Error('Plugs.DosingSystem 类重复加载');
    var pescoDosingSystem = window.Plugs.DosingSystem = function() {
        this._masterElement = null;
    };

    var _pescoDosingSystem = null;

    //获取实例对象
    pescoDosingSystem.getInstance = function() {
        var rtn = null;
        if (_pescoDosingSystem == null) {
            rtn = new window.Plugs.DosingSystem();
            _pescoDosingSystem = rtn;
        } else {
            rtn = _pescoDosingSystem;
        }
        return rtn;
    };


    pescoDosingSystem.getInstance().systemIframeLoader = function(callBack) {
        ...
        function activeMenu() {
            setTimeout(function() {
                ...
                for (var i = 0; i < menuList.length; i++) {
                    if (menuList[i].getAttribute('data-runclassname') == 'Plugs.DosingSystem') {
    ...
    pescoDosingSystem.getInstance().ChangeShowClick = function() {
        //点击菜单后会自动执行此方法
        Plugs.DosingSystem.getInstance().systemIframeLoader(function(systemIframe) {
            // 调用当前iframe中的方法
            // debugger
            systemIframe.contentWindow.oDefault.clearAll();
            systemIframe.contentWindow.oDosing.showDosingSystem();
        })
    };
}();