! function () {
	...
	if (window.Plugs.WaterDeliverySystem) throw new Error('Plugs.WaterDeliverySystem 类重复加载');
	var pescoWaterDeliverySystem = window.Plugs.WaterDeliverySystem = function () {
		this._masterElement = null;
	};

	var _pescoWaterDeliverySystem = null;

	//获取实例对象
	pescoWaterDeliverySystem.getInstance = function () {
		var rtn = null;
		if (_pescoWaterDeliverySystem == null) {
			rtn = new window.Plugs.WaterDeliverySystem();
			_pescoWaterDeliverySystem = rtn;
		} else {
			rtn = _pescoWaterDeliverySystem;
		}
		return rtn;
	};


	pescoWaterDeliverySystem.getInstance().systemIframeLoader = function (callBack) {
		...
		function activeMenu() {
			setTimeout(function () {
				...
				for (var i = 0; i < menuList.length; i++) {
					if (menuList[i].getAttribute('data-runclassname') == 'Plugs.WaterDeliverySystem') {
	...
	pescoWaterDeliverySystem.getInstance().ChangeShowClick = function () {
		//点击菜单后会自动执行此方法
		Plugs.WaterDeliverySystem.getInstance().systemIframeLoader(function (systemIframe) {
			// 调用当前iframe中的方法
			// debugger
			systemIframe.contentWindow.oDefault.clearAll();
			systemIframe.contentWindow.oWaterDelivery.showWaterDeliverySystem();
		})
	};
}();