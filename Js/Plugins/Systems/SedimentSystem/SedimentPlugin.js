! function () {
	...
	if (window.Plugs.SedimentSystem) throw new Error('Plugs.SedimentSystem 类重复加载');
	var pescoSedimentSystem = window.Plugs.SedimentSystem = function () {
		this._masterElement = null;
	};

	var _pescoSedimentSystem = null;

	//获取实例对象
	pescoSedimentSystem.getInstance = function () {
		var rtn = null;
		if (_pescoSedimentSystem == null) {
			rtn = new window.Plugs.SedimentSystem();
			_pescoSedimentSystem = rtn;
		} else {
			rtn = _pescoSedimentSystem;
		}
		return rtn;
	};


	pescoSedimentSystem.getInstance().systemIframeLoader = function (callBack) {
		...
		function activeMenu() {
			setTimeout(function () {
				...
				for (var i = 0; i < menuList.length; i++) {
					if (menuList[i].getAttribute('data-runclassname') == 'Plugs.SedimentSystem') {
	...
	pescoSedimentSystem.getInstance().ChangeShowClick = function () {
		//点击菜单后会自动执行此方法
		Plugs.SedimentSystem.getInstance().systemIframeLoader(function (systemIframe) {
			// 调用当前iframe中的方法
			// debugger
			systemIframe.contentWindow.oDefault.clearAll();
			systemIframe.contentWindow.oSediment.showSedimentSystem();
		})
	};
}();