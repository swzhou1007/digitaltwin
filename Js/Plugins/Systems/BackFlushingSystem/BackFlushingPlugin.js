! function() {
	...
	if (window.Plugs.BackFlushingSystem) throw new Error('Plugs.BackFlushingSystem 类重复加载');
	var pescoBackFlushingSystem = window.Plugs.BackFlushingSystem = function() {
		this._masterElement = null;
	};

	var _pescoBackFlushingSystem = null;

	//获取实例对象
	pescoBackFlushingSystem.getInstance = function() {
		var rtn = null;
		if (_pescoBackFlushingSystem == null) {
			rtn = new window.Plugs.BackFlushingSystem();
			_pescoBackFlushingSystem = rtn;
		} else {
			rtn = _pescoBackFlushingSystem;
		}
		return rtn;
	};


	pescoBackFlushingSystem.getInstance().systemIframeLoader = function(callBack) {
		...
		function activeMenu() {
			setTimeout(function() {
				...
				for (var i = 0; i < menuList.length; i++) {
					if (menuList[i].getAttribute('data-runclassname') == 'Plugs.BackFlushingSystem') {
	...
	pescoBackFlushingSystem.getInstance().ChangeShowClick = function() {
		//点击菜单后会自动执行此方法
		Plugs.BackFlushingSystem.getInstance().systemIframeLoader(function(systemIframe) {
			// 调用当前iframe中的方法
			systemIframe.contentWindow.oDefault.clearAll();
			systemIframe.contentWindow.oBackFlushing.showBackFlushingSystem();
		})
	};
}();
